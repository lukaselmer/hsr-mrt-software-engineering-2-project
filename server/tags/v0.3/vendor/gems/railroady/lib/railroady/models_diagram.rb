...
class ModelsDiagram < AppDiagram
  ...
  def process_association(class_name, assoc)
    ...
    singularized_assoc_class_name = assoc.class_name.underscore
    singularized_assoc_name = assoc.name.to_s
    if assoc.macro.to_s != 'has_one' && singularized_assoc_name != "user_addresses"
      singularized_assoc_class_name = singularized_assoc_class_name.singularize 
      singularized_assoc_name = singularized_assoc_name.singularize
    end

    # from patch #12384
    # if assoc.class_name == assoc.name.to_s.singularize.camelize
    assoc_class_name = (assoc.class_name.respond_to? 'underscore') ? singularized_assoc_class_name.camelize : assoc.class_name
    if assoc_class_name == singularized_assoc_name.camelize
      assoc_name = ''
    else
